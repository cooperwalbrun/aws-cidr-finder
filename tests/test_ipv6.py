...
from aws_cidr_finder import ipv6
...
def test_get_first_ip_in_next_cidr() -> None:
    ...
    for cidr, expected in test_cases:
        assert ipv6._get_first_ip_in_next_cidr(cidr) == expected
...
def test_get_last_ip_in_previous_cidr() -> None:
    ...
    for cidr, expected in test_cases:
        assert ipv6._get_last_ip_in_previous_cidr(cidr) == expected
...
def test_get_previous_cidr_with_prefix() -> None:
    ...
    for cidr, prefix, expected in test_cases:
        assert ipv6._get_previous_cidr_with_prefix(cidr, prefix) == expected
...
def test_get_next_cidr_with_prefix() -> None:
    ...
    for cidr, prefix, expected in test_cases:
        assert ipv6._get_next_cidr_with_prefix(cidr, prefix) == expected
...
def test_cidrs_are_adjacent() -> None:
    ...
    for cidr1, cidr2 in true_test_cases:
        assert ipv6._cidrs_are_adjacent(cidr1, cidr2)
        assert ipv6._cidrs_are_adjacent(cidr2, cidr1)
    ...
    for cidr1, cidr2 in false_test_cases:
        assert not ipv6._cidrs_are_adjacent(cidr1, cidr2)
        assert not ipv6._cidrs_are_adjacent(cidr2, cidr1)
...
def test_break_down_to_desired_prefix() -> None:
    ...
    for cidrs, prefix, expected in test_cases:
        actual = IPv6CIDREngine.break_down_to_desired_prefix(cidrs, prefix)
        _assert_lists_equal(actual[0], expected)

        reversed = cidrs.copy()
        reversed.reverse()  # To assert that order is irrelevant
        actual_reverse = IPv6CIDREngine.break_down_to_desired_prefix(reversed, prefix)
...
def test_find_subnet_holes() -> None:
    ...
    for vpc_cidr, cidrs, expected in test_cases:
        actual = IPv6CIDREngine.find_subnet_holes(vpc_cidr, cidrs)
        _assert_lists_equal(actual, expected)

        reversed = cidrs.copy()
        reversed.reverse()  # To assert that order is irrelevant
        actual_reverse = IPv6CIDREngine.find_subnet_holes(vpc_cidr, reversed)
